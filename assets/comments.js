...
export class PostCounts extends HTMLElement {
  postData = {};
  ...
  async #fetchMastodon(url) {
    ...
    const postData = postDataFromMastodon(
      await fetchJSON(
        new URL(`${origin}/api/v1/statuses/${id}`),
        options,
      ),
    );
  ...
  render(container, postData) {
    container.innerHTML = this.renderComment(postData);
  }

  renderComment(postData) {
    const url = this.getAttribute("mastodon");
    return `
        <div class="py-2 text-secondary">
            <a href="${url}" rel="nofollow">${postData.boosts ? `<span>${icons.reblog} ${postData.boosts}</span></a>` : ""
      }
            <a href="${url}" rel="nofollow">${postData.likes ? `<span>${icons.favourite} ${postData.likes}</span></a>` : ""
      }
        </div>
      `;
  }
...
function postDataFromMastodon(data) {
  return {
...
function dataFromMastodon(data, author, source) {
  ...
  for (const comment of data.descendants) {
    if (comment.visibility !== "public" && comment.visibility !== "unlisted") {
      continue;
    }
